export function renderSidebarNav(activeId: string) {
  ...
  aside.innerHTML = `
    <div class="glass-card p-4 lg:p-6 sticky top-20 lg:top-28 z-40">
      ...
      <nav class="flex flex-row lg:flex-col overflow-x-auto lg:overflow-visible gap-2 pb-2 lg:pb-0 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
        ${items.map(item => `
          <a href="${item.href}" class="px-4 py-2 lg:py-3 rounded-xl text-sm font-medium transition-all whitespace-nowrap ${item.id === activeId ? 'bg-brand-red text-white shadow-lg shadow-brand-red/20' : 'text-white/50 hover:bg-white/5 hover:text-white'}">
            ${item.label}
          </a>
        `).join('')}
      </nav>
    ...
    <div class="glass-card p-6 mt-6 hidden lg:block">
